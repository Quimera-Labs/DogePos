...
#if defined(NDEBUG)
# error "DogePos cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "DogePos Signed Message:\n";
...
CAmount GetProofOfWorkSubsidy()
{
    ...
    if (nBlockHeight > 1 && nBlockHeight <= 65000) {
        return 5 * COIN;
    }

    if (nBlockHeight > 65000 && nBlockHeight <= 130000) {
        return 25 * COIN;
    }

    if (nBlockHeight > 130000 && nBlockHeight <= 195000) {
        return 250 * COIN;
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // DogePos also requires DER encoding of pubkeys
    flags |= SCRIPT_VERIFY_DERKEY;
    // DogePos also requires low S in sigs
