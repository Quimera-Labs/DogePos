...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    if (pnext)
        result.push_back(Pair("nextblockhash", pnext->GetBlockHash().GetHex()));
    result.push_back(Pair("flags", blockindex->IsProofOfStake()? "Pos-DPS" : "Pow-DPS"));
    result.push_back(Pair("modifier", blockindex->nStakeModifier.GetHex()));
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of dogepos addresses\n"
            "        \"dogeposaddress\"     (string) dogepos address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
